...
public sealed class AuditTrailEngine(
    ...
    : IAuditTrailEngine
{
    private readonly ILogger<AuditTrailEngine> _logger = logger ?? throw new ArgumentNullException(nameof(logger));
    private readonly AuditTrailOptions _options = options?.Value ?? new AuditTrailOptions();
    private readonly ConcurrentQueue<AuditEntry> _buffer = new();
    private readonly SemaphoreSlim _flushLock = new(1, 1);
    private readonly IAuditStorage _storage = storage ?? throw new ArgumentNullException(nameof(storage));
